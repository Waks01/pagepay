...
export function TransactionDetailHeader({ transaction, tokens }: TransactionDetailHeaderProps) {
  ...
  const formatAmount = () => {
    ...
    if (transaction.unit === 'NGN') {
      return `₦${(absAmount / 100).toLocaleString('en-NG', {
  ...
  return (
    <View style={[styles.header, { backgroundColor: tokens.card, borderBottomColor: tokens.border }]}>
      ...
      <Text style={[styles.amount, { color: isDebit ? tokens.error : tokens.mint }]}>
        {isDebit ? '-' : '+'}
        {formatAmount()}
      </Text>
