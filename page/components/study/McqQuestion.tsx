...
export function McqQuestion({
  ...
  // Editorial "outline-then-fill" letter chips. Before the user picks,
  // every chip is outlined; once they pick, the chosen chip + the correct
  // chip take on their brand color; the wrong pick flashes signal.
  const optionBgs = options.map((_, idx) => {
    if (!showAnswer) return selected === idx ? tokens.mintFaint : tokens.card;
    if (idx === correct_index) return tokens.mintFaint;
    if (idx === selected && idx !== correct_index) return tokens.signalFaint;
    return tokens.card;
  });
  const optionBorderColors = options.map((_, idx) => {
    if (!showAnswer) return selected === idx ? tokens.mint : tokens.border;
    if (idx === correct_index) return tokens.mint;
    if (idx === selected && idx !== correct_index) return tokens.signal;
    return tokens.border;
  });
  const chipBgs = options.map((_, idx) => {
    if (!showAnswer) return selected === idx ? tokens.mint : tokens.paper2;
    if (idx === correct_index) return tokens.mint;
    if (idx === selected && idx !== correct_index) return tokens.signal;
    return tokens.paper2;
  });
  const chipBorderColors = options.map((_, idx) => {
    if (!showAnswer) return selected === idx ? tokens.mint : tokens.border;
    if (idx === correct_index) return tokens.mint;
    if (idx === selected && idx !== correct_index) return tokens.signal;
    return tokens.border;
  });
  const chipTextColors = options.map((_, idx) => {
    if (!showAnswer) return selected === idx ? tokens.mintText : tokens.inkMuted;
    if (idx === correct_index) return tokens.mintText;
    if (idx === selected && idx !== correct_index) return tokens.mintText;
  ...
  return (
    <Animated.View style={[styles.card, { backgroundColor: tokens.card, borderColor: tokens.border }, cardAnimatedStyle]}>
      ...
      <View style={styles.questionHeader}>
        <Text style={[styles.question, { color: tokens.ink, fontFamily: Fonts.editorialSemiBold as string }]}>{question}</Text>
        {onBookmark && (
          <TouchableOpacity
            onPress={onBookmark}
          ...
          return (
            <AnimatedTouchable
              ...
              style={[
                styles.option,
                {
                  borderColor: optionBorderColors[idx],
                  backgroundColor: optionBgs[idx],
                },
                optionAnimatedStyle,
              ]}
            >
              <View
                style={[
                  styles.chip,
                  {
                    backgroundColor: chipBgs[idx],
                    borderColor: chipBorderColors[idx],
                  },
                ]}
              >
                <Text style={[styles.chipText, { color: chipTextColors[idx] }]}>
