...
export default function BeneficiariesScreen() {
  ...
  const beneficiariesQ = useQuery({
    ...
    queryFn: async () => {
      const path = search
        ? `/api/v1/bills/beneficiaries?q=${encodeURIComponent(search)}`
        : "/api/v1/bills/beneficiaries";
  ...
  const updateMutation = useMutation({
    mutationFn: async ({
      ...
      id: number;
      payload: { name: string; phone: string; network: string };
    }) => {
      ...
      <ScrollView contentContainerStyle={{ padding: 20, gap: 16 }}>
        ...
        {!showForm ? (
          ...
          >
            ...
            <Text style={[styles.addBtnText, { color: tokens.mintText }]}>
              Add Beneficiary
            </Text>
          </TouchableOpacity>
        ) : (
          ...
          >
            <Text style={[styles.formTitle, { color: tokens.ink }]}>
            ...
            <View style={{ flexDirection: "row", gap: 10 }}>
              ...
              >
                <Text style={{ color: tokens.inkMuted, fontWeight: "600" }}>
                  Cancel
                </Text>
              </TouchableOpacity>
              <TouchableOpacity
        ...
        {beneficiariesQ.isLoading ? (
          ...
          <View style={{ gap: 10 }}>
            {list.map((b) => (
              ...
              >
                <View style={{ flex: 1 }}>
                  <Text style={[styles.beneficiaryName, { color: tokens.ink }]}>
                    {b.name}
                  </Text>
                  <Text
                    style={[
                      styles.beneficiaryPhone,
                      { color: tokens.inkMuted },
                    ]}
                  >
