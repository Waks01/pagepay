...
import {
  fetchSponsorTasks,
  type TaskResponseFull,
} from "@/src/features/sponsor/api";
      ...
      >
        <View style={styles.taskHeader}>
          <View
            style={[
              styles.statusBadge,
              { backgroundColor: getStatusColor(item.status, tokens) },
            ]}
          >
            <Text style={[styles.statusText, { color: tokens.mintText }]}>
              {t(`sponsor_dashboard.status_${item.status}`)}
            </Text>
          </View>
          <Text style={[styles.rewardText, { color: tokens.mint }]}>
            {t("sponsor_dashboard.reward_each", {
              amount: (item.reward_amount / 100).toFixed(2),
            })}
          </Text>
        </View>
        ...
        <View style={[styles.statsRow, { borderColor: tokens.border }]}>
          <View style={styles.statItem}>
            <Text style={[styles.statValue, { color: tokens.ink }]}>
              {t("sponsor_dashboard.completed_of", {
                completed: item.completed_count,
                total: item.max_completions,
              })}
            </Text>
            <Text style={[styles.statLabel, { color: tokens.inkMuted }]}>
              {t("sponsor_dashboard.completed_label")}
            </Text>
          </View>

          <View style={styles.statItem}>
            <Text style={[styles.statValue, { color: tokens.ink }]}>
              {t("sponsor_dashboard.pending_count", {
                count: item.pending_count,
              })}
            </Text>
            <Text style={[styles.statLabel, { color: tokens.inkMuted }]}>
              {t("sponsor_dashboard.pending_label")}
            </Text>
          </View>

          <View style={styles.statItem}>
            <Text style={[styles.statValue, { color: tokens.ink }]}>
              {t("sponsor_dashboard.progress_percent", {
                percent: completionRate,
              })}
            </Text>
            <Text style={[styles.statLabel, { color: tokens.inkMuted }]}>
              {t("sponsor_dashboard.progress_label")}
            </Text>
          </View>
        </View>
        ...
        <View style={styles.taskFooter}>
          <View style={styles.taskMeta}>
            ...
            <Text style={[styles.taskMetaText, { color: tokens.inkMuted }]}>
              {t("sponsor_dashboard.expires_date", {
                date: formatDateTime(item.expires_at, { includeTime: false }),
              })}
            </Text>
          </View>
          ...
          <View style={styles.taskMeta}>
            ...
            <Text style={[styles.taskMetaText, { color: tokens.inkMuted }]}>
              {t("sponsor_dashboard.spent_label", {
                amount: (item.total_spent / 100).toFixed(2),
              })}
            </Text>
          </View>
        </View>
          ...
          >
            <Text
              style={[
                styles.filterText,
                {
                  color: filter === status ? tokens.mintText : tokens.inkMuted,
                },
              ]}
            >
        ...
        ListEmptyComponent={
          <View style={styles.emptyContainer}>
            <Ionicons
              ...
              color={tokens.border}
            />
            <Text style={[styles.emptyText, { color: tokens.ink }]}>
              {t("sponsor_dashboard.empty_title")}
            </Text>
            <TouchableOpacity
            ...
            >
              ...
              <Text
                style={[styles.createButtonText, { color: tokens.mintText }]}
              >
                {t("sponsor_dashboard.create_first")}
              </Text>
            </TouchableOpacity>
          </View>
        }
...
function getStatusColor(status: string, tokens: (typeof PagePay)["light"]) {
  switch (status) {
    ...
    case "active":
      return tokens.mint;
    case "paused":
      return "#FFA500"; // keep orange — no dedicated token
    case "completed":
      return tokens.mint;
    case "expired":
