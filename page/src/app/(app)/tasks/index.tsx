...
const CATEGORIES = [
  ...
  {
    key: "social_media",
    labelKey: "tasks.categories.social_media",
    icon: "people-outline",
  },
  {
    key: "engagement",
    labelKey: "tasks.categories.engagement",
    icon: "heart-outline",
  },
  {
    key: "website",
    labelKey: "tasks.categories.website",
    icon: "globe-outline",
  },
  {
    key: "app",
    labelKey: "tasks.categories.app",
    icon: "phone-portrait-outline",
  },
  {
    key: "content_creation",
    labelKey: "tasks.categories.content_creation",
    icon: "create-outline",
  },
  {
    key: "surveys",
    labelKey: "tasks.categories.surveys",
    icon: "clipboard-outline",
  },
  {
    key: "data_collection",
    labelKey: "tasks.categories.data_collection",
    icon: "server-outline",
  },
  {
    key: "other",
    labelKey: "tasks.categories.other",
...
const SORT_OPTIONS = [
  ...
  {
    key: "highest_reward",
    labelKey: "tasks.sort.highest_reward",
    icon: "cash-outline",
  },
  { key: "quickest", labelKey: "tasks.sort.quickest", icon: "flash-outline" },
  {
    key: "popular",
    labelKey: "tasks.sort.popular",
...
export default function TasksScreen() {
  ...
  const filteredTasks = useMemo(() => {
    ...
    switch (sortBy) {
      ...
      case "quickest":
        result.sort(
          (a, b) =>
            (a.time_estimate_minutes || 999) - (b.time_estimate_minutes || 999),
        );
        break;
      case "popular":
  ...
  const renderTask = ({ item }: { item: TaskListItem }) => {
    const netRewardKobo = Math.floor(
      item.reward_amount * (item.reward_multiplier ?? 1) * 0.85,
    );
    ...
    const platformIcon =
      PLATFORM_ICONS[item.platform.toLowerCase()] || "briefcase-outline";
    const taskLabel =
      TASK_TYPE_LABELS[item.task_type] || item.task_type.replace(/_/g, " ");
    const remainingPercent = Math.round(
      (item.completed_count / Math.max(1, item.max_completions)) * 100,
    );
    ...
    return (
      ...
      >
        ...
        <View style={styles.taskHeader}>
          <View style={styles.taskHeaderLeft}>
            ...
            {item.task_source === "admin" && (
              ...
              >
                <Text
                  style={[styles.sourceBadgeText, { color: tokens.signal }]}
                >
                  Official
                </Text>
              </View>
        ...
        {/* Title + description */}
        <Text
          style={[styles.taskTitle, { color: tokens.ink }]}
          numberOfLines={2}
        >
          {item.title}
        </Text>
        <Text
          style={[styles.taskDescription, { color: tokens.inkMuted }]}
          numberOfLines={2}
        >
          {item.description}
        </Text>
        ...
        <View style={styles.progressContainer}>
          <View
            style={[styles.progressBar, { backgroundColor: tokens.border }]}
          >
            <View
              style={[
                styles.progressFill,
        ...
        <View style={styles.taskFooter}>
          ...
          <View style={styles.taskMeta}>
            <Ionicons
              ...
              size={14}
              color={isExpiringSoon ? tokens.signal : tokens.inkMuted}
            />
            <Text
              style={[
                styles.taskMetaText,
            ...
            >
              {formatDateTime(expiresDate.toISOString(), {
                includeTime: false,
              })}
            </Text>
          </View>
          ...
          {item.sponsor_business_name && (
            <View style={styles.taskMeta}>
              ...
              />
              <Text
                style={[styles.taskMetaText, { color: tokens.inkMuted }]}
                numberOfLines={1}
              >
                {item.sponsor_business_name}
              </Text>
